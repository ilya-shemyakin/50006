...
namespace
{
  // Формирует [DBL LIT] с обязательной точкой. Использует минимально
  // необходимую точность, при которой значение корректно читается обратно
  // (round-trip). Гарантирует наличие хотя бы одной цифры после точки.
...
std::istream& operator>>(std::istream& in, DataStruct& dataStruct)
{
  ...
  while (!(hasKey1 && hasKey2 && hasKey3))
  {
    ...
    // Метка поля: только буквы/цифры, без пропуска пробелов
    std::string label;
    int ch = in.peek();
    ...
    // Ровно один пробел между меткой и значением
    if (in.peek() != ' ')
    {
      in.setstate(std::ios::failbit);
      return in;
    ...
    {
      // Неизвестная метка или дубликат поля
      in.setstate(std::ios::failbit);
      return in;
    }
  }
