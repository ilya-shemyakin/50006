...
std::string processInFrame(
    ...
    return isInFrame(polygons, polygon) ? "<TRUE>" : "<FALSE>";
}

std::string processRmEcho(
    std::istringstream& in,
    ...
    return std::to_string(removeEcho(polygons, polygon));
}
...
std::string CommandProcessor::operator()(const CommandLine& commandLine) const {
    try {
        ...
        if (command == "RMECHO") {
            return processRmEcho(in, polygons);
        }
        if (command == "INFRAME") {
            return processInFrame(in, polygons);
        }
        throw std::logic_error("Invalid command");
    }
